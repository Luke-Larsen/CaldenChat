...
bool messageBot(std::string &message);
...
bool Server::ListenForNewConnection()
{
	...
	{
		std::ifstream Reader("MOTD.txt");             //Open file
...
bool Server::ProcessPacket(int ID, Packet _packettype)
{
	...
	case P_ChatMessage: //Packet Type: chat message
	{
		...
		std::fstream log("log.bin", std::ios::out | std::ios::app | std::ios::binary);
		if (log.is_open()) {
...
bool messageBot(std::string &message) {
	std::string messageBot = message;
	std::string messageBotFirst = message;
	std::string messageBotSecound = message;
	std::size_t found = messageBot.find(" : ");
	messageBotFirst.erase(0, found + 3);
	messageBotSecound.erase(found, messageBot.length());
